...
def matrix_divided(matrix, div):
    ...
    """
    if matrix is None or not isinstance(matrix, list) or any(
            not isinstance(row, list) or not all(isinstance(
    ...
    if div == float('inf'):
        new_matrix = [[0.0 for element in row] for row in matrix]
    else:
        new_matrix = [[round(
            element / div, 2) for element in row] for row in matrix]
    return new_matrix
